...
export default function OverviewView({ data, periodActive, onNavigate, onOpenCase, onOpenComplaint }: { data: Workspace; periodActive: boolean; onNavigate: (screen: Destination) => void; onOpenCase: (id: string) => void; onOpenComplaint: (id: string) => void }) {
  ...
  return <div className="reference-overview">
    ...
    <section className="reference-kpis" aria-label="Team activity">
      ...
      <button onClick={() => onNavigate('cases')} className="reference-kpi"><span className="reference-kpi__icon mint"><FlaskConical size={31} /></span><span><small>{periodActive ? 'Screenings in period' : 'Screenings Today'}</small><strong>{periodActive ? data.screening_records.length : testsToday}</strong><em>{periodActive ? 'Field records in selected dates' : `${data.screening_records.length} team screenings`}</em></span></button>
    ...
    <div className="reference-overview__grid">
      ...
      <div className="reference-overview__right">
        <section className="reference-alerts"><header><h2>Alerts &amp; Updates</h2><span>{alerts.length} shown</span><button onClick={() => onNavigate('alerts')}>View All</button></header><div>{alerts.map(alert => <button key={alert.id} className="reference-alert" onClick={() => data.cases.some(record => record.id === alert.id) ? onOpenCase(alert.id) : onOpenComplaint(alert.id)}><span className={`reference-alert__icon ${alert.kind}`}>{alert.kind === 'critical' ? <AlertTriangle size={24} fill="currentColor" /> : <Clock3 size={24} />}</span><span><strong>{alert.title}</strong><small>{alert.subtitle}</small><em>{alert.detail}</em></span><time>{alert.time}</time></button>)}{!alerts.length && <p className="empty-state">No urgent case or phone alerts in these records.</p>}</div></section>
        <section className="reference-tasks"><header><h2>Open Cases</h2><button onClick={() => onNavigate('cases')}>View All</button></header><div>{sorted.slice(0, 4).map((record, index) => <button key={record.id} onClick={() => onOpenCase(record.id)}><span className={`reference-task__icon task-${index % 4}`}>{index % 2 ? <FlaskConical size={19} /> : <ShieldCheck size={19} />}</span><span><strong>{record.origin === 'ivr' ? 'Review phone report' : 'Review field screening'} — {shortId(record.id)}</strong><small>{data.water_sources.find(source => source.id === record.source_id)?.name || 'Water source'}</small></span><em className={record.priority}>{record.priority === 'critical' ? 'Priority' : record.priority === 'urgent' ? 'Soon' : 'Open'}</em></button>)}{!sorted.length && <p className="empty-state">No open cases in these records.</p>}</div></section>
