...
async function server(run:(base:string,setRole:(role:string)=>void,setMode:(mode:string)=>void)=>Promise<void>){
 let role='supervisor'
 let mode='synthetic'
 ...
 try{await run(`http://127.0.0.1:${address.port}`,value=>{role=value},value=>{mode=value})}finally{http.closeAllConnections();await new Promise<void>(resolve=>http.close(()=>resolve()))}
}
