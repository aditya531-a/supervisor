...
async (page) => {
  ...
  await page.getByRole('searchbox',{name:'Search sources and cases'}).fill('Kalyanpur');
  await page.locator('.reference-search__results button').first().click();
  assert(await page.locator('.reference-source-card').count()===1,'Source search should open the matching source');
  await page.getByRole('button',{name:'Show all sources'}).click();
  assert(await page.locator('.reference-source-card').count()===6,'Show all sources should clear the search selection');
  await nav('Overview').click();
  await page.locator('.reference-alerts .reference-alert').last().click();
  await page.waitForFunction(()=>document.activeElement?.id==='complaint-complaint-demo');
  await nav('Overview').click();
  ...
  assert(await page.getByRole('button',{name:/High-Risk Alerts/}).locator('strong').textContent()==='4','Date edits should wait for Apply');
  await page.getByRole('button',{name:'Apply'}).click();
  assert(await page.getByRole('button',{name:/High-Risk Alerts/}).locator('strong').textContent()==='0','Applied dates should filter the overview');
  assert(await page.getByRole('button',{name:/Screenings in period/}).locator('strong').textContent()==='0','Filtered screening count needs a period label');
  ...
  assert(await page.getByRole('button',{name:'All dates'}).isVisible(),'Mobile date filter is hidden');
