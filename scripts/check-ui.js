...
async (page) => {
  ...
  await capture('reference-reports-320');
  await page.getByRole('button',{name:'Open menu'}).click();
  await nav('Cases').click();
  await capture('reference-cases-320-detail');
  await page.getByRole('button',{name:'Back to queue'}).click();
  await capture('reference-cases-320-queue');
  await page.locator('.queue-row').first().click();
  await page.getByRole('button',{name:'Open menu'}).click();
  await nav('Sources').click();
  await capture('reference-sources-320');
  await page.getByRole('button',{name:'Open menu'}).click();
  await nav('Alerts').click();
  await capture('reference-alerts-320');
  await page.getByRole('button',{name:'Open menu'}).click();
  await nav('Settings').click();
  await capture('reference-settings-320');
  await page.setViewportSize({width:1586,height:992});
  failWorkspace=true;
